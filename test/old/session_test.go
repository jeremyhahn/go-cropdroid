...
func NewUnitTestSession() (*app.App, service.Session) {
	...
	session := service.CreateSession(logger, nil, nil, nil, 0, 0, common.CONSISTENCY_LOCAL, &model.UserStruct{
		ID:       1,
...
func createTestFarm(farmID uint64, deviceType string) (*config.FarmStruct, state.DeviceStateMap) {
	...
	condition := &config.ConditionStruct{
		ID:         1,
		ChannelID:  1,
		MetricID:   1,
		Comparator: ">",
	...
	channels := []*config.ChannelStruct{{
		ID:         1,
		DeviceID:   1,
		BoardID:    testChannelID,
		Name:       "test",
		Enable:     true,
		Notify:     true,
		Conditions: []*config.ConditionStruct{condition}}}

	device := &config.DeviceStruct{
		ID:   1,
		Type: deviceType,
	...
	farmConfig := &config.FarmStruct{
		ID: farmID,
		//OrganizationID: 0,
		Mode:     "virtual",
		Name:     "Test Farm",
		Interval: 50,
		Devices:  []*config.DeviceStruct{device}}
