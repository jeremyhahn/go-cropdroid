...
func NewRaftOrganizationDAO(logger *logging.Logger,
	raftNode cluster.RaftNode, clusterID uint64,
...
func (dao *RaftOrganizationDAO) Save(organization *config.Organization) error {
	...
	serverConfig.AddOrganizationRef(organization.ID)
...
func (orgDAO *RaftOrganizationDAO) GetAll(CONSISTENCY_LEVEL int) ([]*config.Organization, error) {
	pageResult, err := orgDAO.GetPage(query.NewPageQuery(), common.CONSISTENCY_LOCAL)
	return pageResult.Entities, err
}

func (orgDAO *RaftOrganizationDAO) GetPage(pageQuery query.PageQuery, CONSISTENCY_LEVEL int) (dao.PageResult[*config.Organization], error) {
	orgDAO.logger.Debugf("Fetching organization page: %v+", pageQuery)
	var result interface{}
	var err error
	...
	if CONSISTENCY_LEVEL == common.CONSISTENCY_LOCAL {
		result, err = orgDAO.raft.ReadLocal(orgDAO.clusterID, query)
		if err != nil {
			orgDAO.logger.Errorf("Error (orgID=%d): %s", orgDAO.clusterID, err)
			return pageResult, err
		}
	} else if CONSISTENCY_LEVEL == common.CONSISTENCY_QUORUM {
		result, err = orgDAO.raft.SyncRead(orgDAO.clusterID, query)
		if err != nil {
			orgDAO.logger.Errorf("Error (orgID=%d): %s", orgDAO.clusterID, err)
			return pageResult, err
		}
	...
	case []*config.Organization:
		pageResult.Entities = v
		return pageResult, nil
	default:
		orgDAO.logger.Errorf("unexpected query type %T", v)
		return pageResult, nil
	}
}
...
func (dao *RaftOrganizationDAO) Delete(organization *config.Organization) error {
	...
	serverConfig.RemoveOrganizationRef(organization.ID)
	if err := dao.serverDAO.Save(serverConfig); err != nil {
		return err
	}
	return nil
