...
type IdGenerator interface {
	NewID([]byte) uint64
	NewID32(key string) int
	NewStringID(key string) uint64
...
func (hasher *Fnv1aHasher) NewID(key []byte) uint64 {
	return hasher.createClusterHash(key)
}

// Returns a new 64-bit FNV-1a hash from a string
func (hasher *Fnv1aHasher) NewStringID(key string) uint64 {
	return hasher.createClusterHash([]byte(key))
}
...
func (hasher *Fnv1aHasher) StringBytes(str string) []byte {
	bytes := make([]byte, 8)
	binary.LittleEndian.PutUint64(bytes, hasher.NewStringID(str))
	return bytes
}
...
func (hasher *Fnv1aHasher) NewUserID(email string) uint64 {
	return hasher.NewStringID(email)
}

func (hasher *Fnv1aHasher) NewRoleID(name string) uint64 {
	return hasher.NewStringID(name)
}
...
func (hasher *Fnv1aHasher) NewWorkflowID(farmID uint64, workflowName string) uint64 {
	return hasher.NewStringID(fmt.Sprintf("%d-%s", farmID, workflowName))
}

func (hasher *Fnv1aHasher) NewWorkflowStepID(workflowID uint64, workflowStepKey string) uint64 {
	return hasher.NewStringID(fmt.Sprintf("%d-%s", workflowID, workflowStepKey))
}
...
func (hasher *Fnv1aHasher) CreateEventLogClusterID(clusterID uint64) uint64 {
	eventLogClusterID := hasher.NewStringID(fmt.Sprintf("%d-%s", clusterID, "eventlog"))
...
func (hasher *Fnv1aHasher) CreateDeviceDataClusterID(deviceID uint64) uint64 {
	deviceDataClusterID := hasher.NewStringID(fmt.Sprintf("%d-%s", deviceID, "devicedata"))
