...
func NewRaftRegistry(logger *logging.Logger,
	idGenerator util.IdGenerator,
	raftNode cluster.RaftNode) dao.Registry {
	...
	eventLogDAO := NewRaftEventLogDAO(logger,
		raftNode, raftOptions.SystemClusterID)
	eventLogDAO.(RaftEventLogDAO).StartClusterNode(false)
	...
	roleDAO.(RaftRoleDAO).StartClusterNode(false)
...
func (registry *RaftDaoRegistry) GetServerDAO() dao.ServerDAO {
