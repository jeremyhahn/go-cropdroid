...
const (
	appliedIndexKey    string = "applied_index"
...
func (r *pebbledb) lookup(key []byte) ([]byte, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	if r.closed {
...
func (r *pebbledb) lookupBatch(keys [][]byte) ([]interface{}, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	entities := make([]interface{}, len(keys))
	if r.closed {
		return nil, errors.New("db already closed")
	}
...
func getNodeDBDirName(dbPath string, clusterID uint64, nodeID uint64) string {
	part := fmt.Sprintf("%d_%d", clusterID, nodeID)
	return filepath.Join(dbPath, part)
}
...
type DiskKV[E interface{}] struct {
...
func NewDiskKV[E interface{}](dbPath string,
	clusterID, nodeID uint64, idGenerator util.IdGenerator) sm.IOnDiskStateMachine {

	diskkv := new(DiskKV[E])
	diskkv.clusterID = clusterID
	diskkv.nodeID = nodeID
	diskkv.dbPath = dbPath
	diskkv.idGenerator = idGenerator
	return diskkv
}

func (d *DiskKV[E]) queryAppliedIndex(db *pebbledb) (uint64, error) {
...
func (d *DiskKV[E]) Open(stopc <-chan struct{}) (uint64, error) {
	dir := getNodeDBDirName(d.dbPath, d.clusterID, d.nodeID)
...
func (d *DiskKV[E]) Lookup(key interface{}) (interface{}, error) {
...
func (d *DiskKV[E]) LookupBatch(keys [][]byte) ([]interface{}, error) {
	db := (*pebbledb)(atomic.LoadPointer(&d.db))
	if db != nil {

		entities, err := db.lookupBatch(keys)
		if err == nil && d.closed {
			panic("lookup returned valid result when DiskKV is already closed")
		}
		if err == pebble.ErrNotFound {
...
func (d *DiskKV[E]) Update(ents []sm.Entry) ([]sm.Entry, error) {
	if d.aborted {
...
// Delete deletes and item from the state machine. In this example, all deletes
// are atomically written to the DB together with the index of the last Raft Log entry.
// For simplicity, we always Sync the writes (db.wo.Sync=True). To get higher throughput,
// you can implement the Sync() method below and choose not to synchronize for every Update().
// Sync() will periodically called by Dragonboat to synchronize the state.
func (d *DiskKV[E]) Delete(ent sm.Entry) error {
	if d.aborted {
		panic("delete() called after abort set to true")
	}
	if d.closed {
		panic("delete called after Close()")
	}
	db := (*pebbledb)(atomic.LoadPointer(&d.db))
	dataKV := &KVData{}
	if err := json.Unmarshal(ent.Cmd, dataKV); err != nil {
		panic(err)
	}
	db.db.Delete([]byte(dataKV.Key), db.syncwo)
	ent.Result = sm.Result{Value: uint64(len(ent.Cmd))}
	// save the applied index to the DB.
	appliedIndex := make([]byte, 8)
	binary.LittleEndian.PutUint64(appliedIndex, ent.Index)
	db.db.Set([]byte(appliedIndexKey), appliedIndex, db.syncwo)
	if d.lastApplied >= ent.Index {
		panic("lastApplied not moving forward")
	}
	d.lastApplied = ent.Index
	return nil
}
...
// Sync() method below and choose not to synchronize for every Update(). Sync()
// will periodically called by Dragonboat to synchronize the state.
func (d *DiskKV[E]) UpdateWithTimeSeriesIndex(ents []sm.Entry, entities []config.TimeSeriesIndexeder) ([]sm.Entry, error) {
	if d.aborted {
		panic("update() called after abort set to true")
	}
	if d.closed {
	...
	for idx, _ := range ents {
		...
		// Store the entity
		wb.Set(timeSeriesIndex.EntityIDKey, jsonEntity, db.wo)
		ents[idx].Result = sm.Result{Value: uint64(len(ents[idx].Cmd))}
	}
...
// you can implement the Sync() method below and choose not to synchronize for every Update().
// Sync() will periodically called by Dragonboat to synchronize the state.
func (d *DiskKV[E]) DeleteWithTimeSeriesIndex(ent sm.Entry, entity config.TimeSeriesIndexeder) error {
	if d.aborted {
		panic("delete() called after abort set to true")
	}
	if d.closed {
	...
	// Delete the TimeSeries record
	if err := db.db.Delete(timeSeriesIndex.TimestampKey, db.syncwo); err != nil {
		panic(err)
	}
	// Delete the entity record
	db.db.Delete(timeSeriesIndex.EntityIDKey, db.syncwo)

	ent.Result = sm.Result{Value: ent.Index}
	//ent.Result = sm.Result{Value: uint64(len(ent.Cmd))}
...
// achieve that.
func (d *DiskKV[E]) PrepareSnapshot() (interface{}, error) {
	if d.closed {
...
func (d *DiskKV[E]) SaveSnapshot(ctx interface{},
...
func (d *DiskKV[E]) saveToWriter(db *pebbledb,
...
func (d *DiskKV[E]) RecoverFromSnapshot(r io.Reader,
