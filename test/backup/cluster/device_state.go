...
func NewRaftDeviceStateStore(logger *logging.Logger, raftCluster cluster.RaftNode) *RaftDeviceStateStore {
...
func (s *RaftDeviceStateStore) Get(clusterID uint64) (state.DeviceStateMap, error) {
	...
	if err != nil {
		s.logger.Errorf("[RaftDeviceStateStore.Get] Error (deviceStateID=%d): %s", clusterID, err)
		return nil, err
	}
...
/* Implements datastore.DeviceDataStore */
...
// 	s.logger.Errorf("Not implemented; expected PUT instead")
// 	return nil
// }
