...
func (dao *MockChannelDAO) Create(channel *config.ChannelStruct) error {
	args := dao.Called(channel)
	fmt.Printf("Creating channel record")
	return args.Error(0)
}

func (dao *MockChannelDAO) Save(channel *config.ChannelStruct) error {
	args := dao.Called(channel)
	fmt.Printf("Saving channel record")
	return args.Error(0)
}

func (dao *MockChannelDAO) Update(channel *config.ChannelStruct) error {
	args := dao.Called(channel)
	fmt.Printf("Updating channel record")
	return args.Error(0)
}

func (dao *MockChannelDAO) Get(channelID int) (*config.ChannelStruct, error) {
	args := dao.Called(channelID)
	fmt.Printf("Getting channel channelID=%d", channelID)
	return args.Get(0).(*config.ChannelStruct), nil
}

func (dao *MockChannelDAO) GetByDeviceNameAndID(deviceID int, name string) (*config.ChannelStruct, error) {
	args := dao.Called(deviceID, name)
	fmt.Printf("Getting channel by device name and ID: name=%s, deviceID=%d", name, deviceID)
	return args.Get(0).(*config.ChannelStruct), nil
}

func (dao *MockChannelDAO) GetByDeviceID(deviceID int) ([]*config.ChannelStruct, error) {
	args := dao.Called(deviceID)
	fmt.Printf("Getting channel by device ID: deviceID=%d", deviceID)
	return args.Get(0).([]*config.ChannelStruct), nil
}

func (dao *MockChannelDAO) GetByUserOrgAndDeviceID(user model.User, deviceID int) ([]*config.ChannelStruct, error) {
