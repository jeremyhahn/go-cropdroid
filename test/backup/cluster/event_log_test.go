...
func TestEventLogCRUD(t *testing.T) {
	...
	assert.Equal(t, "this is a test log entry", page1.Entities[0].Message)
	//assert.Equal(t, timestamp1, eventLogRecords[0].GetTimestampAsObject())

	descPageQuery := query.NewPageQuery()
	descPageQuery.SortOrder = query.SORT_DESCENDING
	page1, err = eventLogDAO.GetPage(descPageQuery, common.CONSISTENCY_LOCAL)
	assert.Nil(t, err)
	assert.NotNil(t, page1.Entities)
	assert.Equal(t, 2, len(page1.Entities))
